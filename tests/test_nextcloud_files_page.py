from pages.nextcloud_files_page import NextcloudFilesPage
from settings import *


def test_nextcloud_files_delete_empty_folder(nextcloud_auth, selenium):
    """Валидация подтверждения действий при удалении пустого каталога через раздел 'Действия'."""
    page = NextcloudFilesPage(selenium)
...
def test_nextcloud_files_cancel_delete_not_empty(nextcloud_auth, selenium):
...
def test_nextcloud_files_close_delete_two_files(nextcloud_auth, selenium):
...
def test_nextcloud_files_menu_delete_file(nextcloud_auth, selenium):
